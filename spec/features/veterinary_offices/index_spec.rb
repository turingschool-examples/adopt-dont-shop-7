...
RSpec.describe "the veterinary offices index" do
  let!(:vet_office_1) {VeterinaryOffice.create!(name: "Special Friends", boarding_services: true, max_patient_capacity: 100)}
  let!(:vet_office_2) {VeterinaryOffice.create!(name: "Pet Emergency Room", boarding_services: true, max_patient_capacity: 50)}
  let!(:vet_office_3) {VeterinaryOffice.create!(name: "The Country Vet", boarding_services: true, max_patient_capacity: 200)}
  let!(:vet_1) {vet_office_1.veterinarians.create!(name: "Morgan", on_call: true, review_rating: 10)}
  let!(:vet_2) {vet_office_1.veterinarians.create!(name: "Heather", on_call: true, review_rating: 9)}
  let!(:vet_3) {vet_office_3.veterinarians.create!(name: "John", on_call: true, review_rating: 9)}
